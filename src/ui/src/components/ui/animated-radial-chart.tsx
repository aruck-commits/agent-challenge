...
export function AnimatedRadialChart({
  ...
  const progressStops = theme === "orion"
    ? ["#2dff9a", "#00ff7f", "#00c97a"]
    : ["#f97316", "#ea580c", "#dc2626"];

  const textStops = theme === "orion"
    ? ["#eafff4", "#b7ffd8", "#55d6a3"]
    : ["#ffffff", "#d1d5db", "#6b7280"];

  const labelColor = theme === "orion" ? "#86efac" : "#9ca3af";
    ...
    <div className={cn("relative", className)} style={{ width: size, height: size * 0.7 }}>
      <svg width={size} height={size * 0.7} viewBox={`0 0 ${size} ${size * 0.7}`} className="overflow-visible">
        <defs>
          ...
          <linearGradient id={`progressGradient-${size}`} x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <stop offset="50%" stopColor={progressStops[1]} />
            <stop offset="100%" stopColor={progressStops[2]} />
          </linearGradient>

          <linearGradient id={`textGradient-${size}`} x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor={textStops[0]} stopOpacity="0.82" />
            <stop offset="50%" stopColor={textStops[1]} stopOpacity="0.6" />
            <stop offset="100%" stopColor={textStops[2]} stopOpacity="0.35" />
          </linearGradient>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        ...
        >
          <span
            style={{
              background: `linear-gradient(to right, ${textStops[0]}, ${textStops[1]})`,
